...
import {SousCategorieActionTypes} from '../../store/sous-categorie.actions';
...
export class MainComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.saveSuccessSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.SaveSousCategorieSuccess),
      map(value => {
    ...
    this.deleteSuccessSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.DeleteSousCategorieSuccess),
      map(value => {
    ...
    this.updateSuccessSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.UpdateSousCategorieSuccess),
      map(value => {
    ...
    this.saveFailedSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.SaveSousCategorieFailed),
      map(value => {
    ...
    this.deleteFailedSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.DeleteSousCategorieFailed),
      map(value => {
    ...
    this.updateFailedSubscription = this.actions.pipe(ofType(
      SousCategorieActionTypes.UpdateSousCategorieFailed),
      map(value => {
