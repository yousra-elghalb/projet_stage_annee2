...
import {MarqueVehiculeActionMarques} from '../../store/marque-vehicule.actions';
...
export class MainComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.saveSuccessSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.SaveMarqueVehiculeSuccess),
      map(value => {
    ...
    this.deleteSuccessSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.DeleteMarqueVehiculeSuccess),
      map(value => {
    ...
    this.updateSuccessSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.UpdateMarqueVehiculeSuccess),
      map(value => {
    ...
    this.saveFailedSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.SaveMarqueVehiculeFailed),
      map(value => {
    ...
    this.deleteFailedSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.DeleteMarqueVehiculeFailed),
      map(value => {
    ...
    this.updateFailedSubscription = this.actions.pipe(ofType(
      MarqueVehiculeActionMarques.UpdateMarqueVehiculeFailed),
      map(value => {
