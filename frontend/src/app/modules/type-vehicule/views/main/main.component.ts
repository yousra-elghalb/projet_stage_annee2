...
import {TypeVehiculeActionTypes} from '../../store/type-vehicule.actions';
...
export class MainComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.saveSuccessSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.SaveTypeVehiculeSuccess),
      map(value => {
    ...
    this.deleteSuccessSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.DeleteTypeVehiculeSuccess),
      map(value => {
    ...
    this.updateSuccessSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.UpdateTypeVehiculeSuccess),
      map(value => {
    ...
    this.saveFailedSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.SaveTypeVehiculeFailed),
      map(value => {
    ...
    this.deleteFailedSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.DeleteTypeVehiculeFailed),
      map(value => {
    ...
    this.updateFailedSubscription = this.actions.pipe(ofType(
      TypeVehiculeActionTypes.UpdateTypeVehiculeFailed),
      map(value => {
