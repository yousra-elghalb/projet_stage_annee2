...
import {SocieteActionTypes} from '../../store/societe.actions';
...
export class MainComponent implements OnInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.saveSuccessSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.SaveSocieteSuccess),
      map(value => {
    ...
    this.deleteSuccessSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.DeleteSocieteSuccess),
      map(value => {
    ...
    this.updateSuccessSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.UpdateSocieteSuccess),
      map(value => {
    ...
    this.saveFailedSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.SaveSocieteFailed),
      map(value => {
    ...
    this.deleteFailedSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.DeleteSocieteFailed),
      map(value => {
    ...
    this.updateFailedSubscription = this.actions.pipe(ofType(
      SocieteActionTypes.UpdateSocieteFailed),
      map(value => {
